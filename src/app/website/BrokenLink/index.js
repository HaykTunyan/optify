...
const WebsiteMetricsBrokenLink = withLazy(import('components/WebsiteMetricsBrokenLink'));
...
const BrokenLink = () => {
  ...
  const { register, handleSubmit, errors, control, watch, reset } = useForm({
    resolver: yupResolver(schemas.brokenLink(t)),
    mode: 'onSubmit',
  });
  ...
  return (
    <Fragment>
      <Helmet>
        <title>{`${t('broken_link')} | ${t('brand_name')}`}</title>
      </Helmet>
      ...
      >
        ...
        <div className='pt-8' />
        <WebsiteMetricsBrokenLink t={t} register={register} errors={errors} control={control} />
        <div className='pt-8' />
